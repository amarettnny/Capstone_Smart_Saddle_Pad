...
static const char *TAG_LISTEN = "twai_listen";
...
{
    ESP_EARLY_LOGW(TAG_LISTEN, "bus error: 0x%x", edata->err_flags.val);
    return false;
}
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG_LISTEN, "TWAI node created");
    ...
    ESP_LOGI(TAG_LISTEN, "Filter enabled for ID: 0x%03X Mask: 0x%03X", data_filter.id, data_filter.mask);
    ...
    ESP_LOGI(TAG_LISTEN, "TWAI start listening...");
    ...
    while (1) {
        if (xSemaphoreTake(twai_listener_ctx.rx_result_semaphore, portMAX_DELAY) == pdTRUE) {
            ...
            ESP_LOGI(TAG_LISTEN, "RX: timestamp %llu, %x [%d] %x %x %x %x %x %x %x %x", \
